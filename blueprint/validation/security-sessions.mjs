// Real Supabase Auth/REST/Storage tests using normal user sessions only.
// Copy to tests/security/rls.sessions.mjs and copy fixture.jpg beside it.
// Requires the supplied schema, two separately provisioned users, Node >=22.
...
if(missing.length || process.env.ALLOW_SECURITY_TESTS!=='1') {
  console.error(`NOT RUN: set ${missing.join(', ') || 'the test configuration'} and ALLOW_SECURITY_TESTS=1 for a disposable project.`);
  process.exit(2);
}
if(process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_SECRET_KEY) {
  console.error('REFUSED: the user-session test process must not contain a service secret.');process.exit(2);
}
const base=process.env.SUPABASE_URL.replace(/\/$/,'');
const host=new URL(base).hostname;
if(!['localhost','127.0.0.1','::1','[::1]'].includes(host) && process.env.ALLOW_REMOTE_TEST_PROJECT!=='1') {
  console.error('REFUSED: remote test project requires ALLOW_REMOTE_TEST_PROJECT=1; do not use production.');process.exit(2);
}
const key=process.env.SUPABASE_PUBLISHABLE_KEY;
function claims(token){try{return JSON.parse(Buffer.from(token.split('.')[1],'base64url').toString());}catch{return {};}}
if(key.startsWith('sb_secret_') || claims(key).role==='service_role') {
  console.error('REFUSED: use a publishable or anon key, never a service key.');process.exit(2);
}
...
async function call(token,path,{method='GET',body,bytes=false,returnRepresentation=false}={}) {
  const r=await fetch(base+path,{method,headers:{apikey:key,...(token?{Authorization:`Bearer ${token}`}:{ }),'Content-Type':bytes?'image/jpeg':'application/json',...(returnRepresentation?{Prefer:'return=representation'}:{})},
    ...(body!==undefined?{body:bytes?body:JSON.stringify(body)}:{}),cache:'no-store',signal:AbortSignal.timeout(15000)});
...
try {
  ...
  passed.push('Anonymous tables/export and unapproved public signup denied');
} catch {
