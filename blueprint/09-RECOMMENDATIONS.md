...
Colour palette (COL1, 21 codes): black, white, cream, grey, navy, blue, light_blue, teal, green, olive, khaki, beige, brown, burgundy, red, yellow, orange, pink, purple, gold, silver; anything else is unknown. Pair score: a neutral (black/white/cream/grey/navy/beige/khaki/brown) with any known colour=1; identical non-neutral hues=0.8; explicitly adjacent pairs `{blue,light_blue}`, `{blue,teal}`, `{light_blue,teal}`, `{teal,green}`, `{green,olive}`, `{red,burgundy}`, `{burgundy,purple}`, `{red,orange}`, `{orange,yellow}`, `{pink,purple}`, `{gold,yellow}`=0.85. Other known pairs=0.6. Colourful mode (contrast pairs `{blue,orange}`, `{purple,yellow}`, `{green,red}` at 0.85) is not active in rules-v1 because it depends on the hidden style preferences. Pair order is irrelevant. Nothing here hard-bans a colour; users can explicitly exclude item combinations.

## rules-v1 as built (I14, ADR21)

I14 implements this file in `src/domain/recommendations.ts` and `src/domain/colour-pairs.ts` with these v1 choices. Where they differ from the general rules above, this section controls rules-v1.

- **No hidden preferences.** Style preferences (preferred colours, style tags, excluded categories, coverage minimums, cold sensitivity, repeat gap) are not rendered since ADR20 and are not an engine input: S is inactive, C uses mean pair compatibility only, category exclusion and coverage minimums are not applied, and the warmth target has no cold-sensitivity offset. The item-level `wear_more` flag is also unused; `exclude_suggestions` stays a hard exclusion.
- **No wear history.** I12/I13 are backlog (PR #37 comment 5815445262), so N and U are inactive and bucket ranking uses season match, occasion fit (unknown formality = 0.5) and favourite, then UUID.
- **Active components:** W is season match (fraction of clothing pieces tagged for the season) unless a temperature is supplied and every clothing piece has known warmth; O uses known core formality only (none known → inactive); C is the mean over pairs of the first colour of each clothing piece (footwear/accessories ignored; fewer than two clothing pieces → inactive); F; P is 0.75 for an exactly liked combination, otherwise 0.5.
- **Weather is inert.** Temperature limits, cold, rain and wind rules apply only to an `outdoors` context with the relevant input, which Phase 5 will supply. Unknown coverage or protection returns a partial result naming the detail.
- **Slots.** Accessories are never suggested. With no weather requirement, an optional layer and outerwear are considered only in autumn/winter, and kept only if they strictly raise the score. The business/formal gate filters core buckets to known formality ≥ target−1 before truncation; unknown core formality is reported as a missing detail.
- **Bounded work and choice.** 8 per category, beam 40, one 2,000-expansion budget per pass shared by both templates, and a second pass at 16 per category only when the first finds nothing and a bucket was truncated. Results are sorted by full-precision score then combination key; up to three are chosen greedily with two different core items, then one. "Show other ideas" passes the already shown core keys as `skip`. Shown core keys, the business/formal dress-code gate and exact dislikes are removed before a beam is trimmed, so paging and heavy dislikes never hide unseen combinations. After the core is complete, acceptable states and disliked cores are kept in two separate beams of 40; a disliked core is only a seed for an added layer or outerwear, and its extension joins the acceptable beam only when that exact combination is allowed. Only the acceptable beam produces results. The partial start uses the same search, filters and bounded second pass, and returns `none` once every start is shown or disliked. Reasons are at most two of season, occasion, colours, liked and favourite.
- **Status:** `empty` (no eligible items), `partial` (no template can be filled; the template with the most filled slots is shown with its missing categories), `none` (everything was rejected or already shown), otherwise `ideas`.
