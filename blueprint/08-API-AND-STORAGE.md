...
| 4. Crop | Accessible crop frame on the photo. Drag the frame to move it; pointer and touch can drag its corner handles to resize. Arrow keys move it and Shift moves it farther; exact percentage values under **Enter exact values** are the keyboard and screen-reader resize route. Rotate turns a quarter turn clockwise; Reset restores the whole photo in its original orientation. Done with no change closes without new work; a changed Done prepares one new photo locally and starts at most one analysis request. Do not require pinch or drag. Default preserves the full garment with a warm neutral background. |
