...
Photo editing (S05): **Edit photo** opens the editor in place of the photo and hides the other photo actions. Drag the frame to move it; pointer and touch can drag its corners to resize. Arrow keys move it (Shift moves farther) and **Enter exact values** holds Left, Top, Width and Height as the keyboard and screen-reader resize route. **Rotate** turns a quarter turn clockwise; **Reset** restores the whole photo in its original orientation. Moving, resizing, rotating, resetting and **Cancel editing** start no analysis and leave the current analysis request as it is; the one existing exception is that cancelling while a changed **Done** is still preparing the new photo stops automatic status checks, and the manual check stays available. **Done** with no change only closes the editor. A changed **Done** prepares one new photo locally and, once that preparation succeeds, starts at most one analysis request for it; results for the previous photo are ignored. Editing never writes to the library, and Save waits until editing is finished.
