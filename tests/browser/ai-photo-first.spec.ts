...
if (process.env.STILLROOM_LINUX_WIRE_PROBE === 'pr30-linux-wire-once-v1' && process.platform === 'linux') {
test.describe('raw-analysis linux differential', () => {
  test.describe.configure({ retries: 0 });
  type Arm = 'retargeted' | 'plain-continue' | 'direct';
  async function sendDirectBrowser(page: Page, api: AiFixture, target: string) {
    return page.evaluate(async ({ authorization, ids, target, key }) => {
      const headers = {
        authorization, apikey: key, 'content-type': 'image/jpeg',
        'x-stillroom-request-id': ids.requestId, 'x-stillroom-draft-id': ids.draftId,
        'x-stillroom-generation': ids.generation,
      };
      const body = new Blob([Uint8Array.from({ length: 4096 }, (_, index) => index % 256)],
        { type: headers['content-type'] });
      const constructedBytes = body.size;
      try {
        const response = await fetch(target, {
          method: 'POST', headers, body, credentials: 'omit', signal: AbortSignal.timeout(5000),
        });
        return { status: response.status, outcome: 'response', constructedBytes };
      } catch { return { status: null, outcome: 'network-rejection', constructedBytes }; }
    }, { authorization: api.issuedWireAuthorization('a'), ids: analysisIds(), target, key: fixtureKey });
  }
    for (const [name, arm] of [
      ['A-retargeted', 'retargeted'], ['B-plain-continue', 'plain-continue'], ['C-direct', 'direct'],
    ] satisfies Array<[string, Arm]>) {
      test(name, async ({ page }, testInfo) => {
        let api: AiFixture | undefined;
        const results: Array<Awaited<ReturnType<typeof sendBrowserAnalysis>> | null> = [null, null, null, null];
        const evidence = rawAnalysisEvidence('response-sequence');
        let plainPostContinuations = 0, hashesVerified = 0;
        let setupCompleted = false, sequenceCompleted = false, consentNoDispatch = false, cleanupFailed = false;
        let storageUnchanged = false, libraryEmpty = false, receiverListening = false;
        let failed = false, failure: unknown;
        try {
          api = await aiFixture(page, 'en', true, undefined, true);
          const observation = api.rawAnalysisObservation;
          if (!observation) throw new Error('Diagnostic observation unavailable.');
          observation.detail = { routePosts: 0, receiverPosts: 0, overflow: false, evidenceError: false,
            routes: [], receivers: [] };
          const { target, origin } = analysisTarget(page, api);
          expect(target.port).not.toBe('54321');
          expect(target.port).not.toBe(new URL(origin).port);
          const inertUrl = origin + '/__raw-analysis-diagnostic__';
          await page.route(inertUrl, async (route) => {
            expect(route.request().method()).toBe('GET');
            await route.fulfill({ status: 200, contentType: 'text/html',
              body: '<!doctype html><html><body><p id="raw-analysis-diagnostic">Synthetic transport</p></body></html>' });
          });
          await page.goto(inertUrl);
          expect(page.url()).toBe(inertUrl);
          expect(new URL(page.url()).origin).toBe(origin);
          await expect(page.locator('#raw-analysis-diagnostic')).toHaveText('Synthetic transport');
          await page.unroute(inertUrl);
          expect(await page.evaluate(async () => {
            if (!('serviceWorker' in navigator)) return { controlled: false, registrations: 0 };
            return { controlled: navigator.serviceWorker.controller !== null,
          ...
          if (arm !== 'retargeted') {
            ...
            if (arm === 'plain-continue') {
              await page.route(target.href, async (route) => {
                expect(route.request().url()).toBe(target.href);
                expect(['OPTIONS', 'POST']).toContain(route.request().method());
                if (route.request().method() === 'POST') plainPostContinuations++;
          ...
          setupCompleted = true;
          const before = storageCounts(api);
          const analysisCalls = () => api!.calls.filter((call) => call.route === analysisPath).length;
          expect(api.inputs).toHaveLength(0);
          expect(analysisCalls()).toBe(0);
          expect(api.results.size).toBe(0);
          const expectedBytes = Buffer.from(Array.from({ length: 4096 }, (_, index) => index % 256));
          for (const [index, status] of [403, 502, 504, 200].entries()) {
            api.consent.set(owners.a, index !== 0);
            if (index === 1) api.mode('failed');
            if (index === 2) api.mode('timeout');
            if (index === 3) api.mode('ready');
            results[index] = arm === 'retargeted'
              ? await sendBrowserAnalysis(page, api, 'valid', undefined, true)
              : await sendDirectBrowser(page, api, target.href);
            expect(results[index]).toEqual({ status, outcome: 'response', constructedBytes: 4096 });
            expect(observation.posts[index]).toMatchObject({ receivedBytes: 4096, acceptedBytes: 4096 });
            expect(api.analysisWire).toMatchObject({ posts: index + 1, callbacks: index + 1,
              receivedBytes: (index + 1) * 4096, payloadBytes: (index + 1) * 4096 });
            expect(api.inputs).toHaveLength(index);
            expect(analysisCalls()).toBe(index);
            if (index === 0) {
              expect(api.results.size).toBe(0);
              consentNoDispatch = true;
            } else {
              assertAnalysisBytes(api.inputs[index - 1]!, expectedBytes);
              hashesVerified++;
            }
          }
          expect(api.analysisWire).toMatchObject({ posts: 4, callbacks: 4, rejected: 0, timedOut: 0, active: 0,
            receivedBytes: 16384, payloadBytes: 16384, routeRejected: 0, forwarded: arm === 'retargeted' ? 4 : 0 });
          expect(hashesVerified).toBe(3);
          expect(api.uploadWire.listening).toBe(true);
          ...
          expect(api.items).toHaveLength(0); expect(api.images).toHaveLength(0); expect(api.files.size).toBe(0);
          libraryEmpty = true;
          expect(observation.posts).toHaveLength(4);
          expect(observation.posts.map(({ receivedBytes, acceptedBytes }) => ({ receivedBytes, acceptedBytes })))
            .toEqual(Array.from({ length: 4 }, () => ({ receivedBytes: 4096, acceptedBytes: 4096 })));
          for (const [index, status] of [403, 502, 504, 200].entries()) {
            expect(observation.posts[index]).toMatchObject({ writeStatus: status,
              postTerminalRejects: 0, firstPostTerminalReject: null });
          }
          expect(observation).toMatchObject({ postCount: 4, overflow: false, evidenceError: false,
            firstAttemptedPost400: null, detail: { routePosts: arm === 'retargeted' ? 4 : 0,
              receiverPosts: 4, overflow: false, evidenceError: false } });
          expect(observation.detail.routes).toHaveLength(arm === 'retargeted' ? 4 : 0);
          for (const [index, record] of observation.detail.routes.entries()) {
            expect(record.ordinal).toBe(index + 1);
            expect(['absent', 'empty', '4096', 'other']).toContain(record.body);
          }
          expect(plainPostContinuations).toBe(arm === 'plain-continue' ? 4 : 0);
          ...
          evidence.project = testInfo.project.name === 'webkit-photo' ? 'webkit-photo' : null;
          evidence.retry = testInfo.retry;
          evidence.repeat = testInfo.repeatEachIndex;
          if (evidence.project === null || evidence.retry !== 0 || evidence.repeat !== 0) evidence.captureError = true;
          try {
            const description = JSON.stringify({ arm, setupCompleted, sequenceCompleted,
              consentNoDispatch, hashesVerified, plainPostContinuations, storageUnchanged, libraryEmpty,
              receiverListening, cleanupFailed, platform: process.platform, node: process.version,
              browser: page.context().browser()?.version() ?? null, evidence });
            expect(Buffer.byteLength(description)).toBeLessThanOrEqual(8192);
            testInfo.annotations.push({ type: 'synthetic-raw-analysis-linux-differential', description });
            console.log('synthetic-raw-analysis-linux-differential', description);
          } catch (error) {
            if (!failed) { failed = true; failure = error; }
          }
        }
        if (failed) throw failure;
        expect(evidence.captureError).toBe(false);
        expect(evidence.cleanupCompleted).toBe(true);
      });
    }
});
}
